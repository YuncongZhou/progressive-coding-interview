"""
Inventory Management System - Stage 4

Multi-user inventory tracking with duplicate item support.
...
- add_duplicate_items creates '<name>.dupe' with half quantity (rounded down)
- remove_duplicate_items merges the dupe back into the original
"""
...
class InventoryManager:
    ...
    def add_duplicate_items(self, user_id: str, name: str) -> str | None:
        """
        Creates a duplicate item named '<name>.dupe' with half the quantity
        (rounded down).
        ...
        Returns:
            New name ('<name>.dupe') or None if original doesn't exist
        ...
        original_qty = user["items"][name]
        dupe_qty = original_qty // 2
        dupe_name = f"{name}.dupe"

        # Check capacity
        current_total = sum(user["items"].values())
        if current_total + dupe_qty > user["capacity"]:
            return None

        user["items"][dupe_name] = dupe_qty
        return dupe_name

    def remove_duplicate_items(self, user_id: str, name: str) -> bool:
        """
        Merges '<name>.dupe' back into '<name>', adding the quantities together.
        ...
        if name not in user["items"]:
            return False
        if dupe_name not in user["items"]:
            return False

        # Merge dupe into original
        user["items"][name] += user["items"][dupe_name]
        del user["items"][dupe_name]
        return True
